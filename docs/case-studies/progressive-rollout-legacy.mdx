...
4. Try `Product Name (external POST)`: in NextGen it updates reactively without full reload; in legacy, reload is required to observe postback behavior.
5. Enable `Trigger failure 🔥` (NextGen only), reload, and confirm automatic fallback to legacy in the server log (forced by error boundary).
...
1. `routingModeFeatureFlag` is exposed by the playground switch.
2. `?legacy=true` remains available as an operational escape hatch.
3. Showing both experiences in parallel (NextGen vs Hapi) validates that `resolveRouteController` keeps routing complexity centralized.
