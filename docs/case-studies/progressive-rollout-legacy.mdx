...
We needed to ship a modern stack incrementally while keeping legacy pages stable and revenue-safe.

We needed surgical control over rollout blast radius:
...
5. Teams needed observability to prove routing decisions in real traffic.
...
2. Preserving `.mjs` keeps parity with real migration constraints (team tooling, runtime conventions, file contracts).
